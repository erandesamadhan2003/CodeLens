  ...
  return (
    <div className="relative min-h-screen bg-paper text-ink flex flex-col font-sans">
      ...
      <header className="relative z-10 w-full px-6 md:px-12 py-6 flex items-center gap-6 border-b-4 border-ink bg-paper">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-display font-bold text-3xl uppercase tracking-tight truncate block">Security (Infilra)</div>
          {runInfo && <div className="font-mono text-sm text-muted mt-1 truncate bg-surface px-3 py-1 inline-block rounded-full border border-ink/20">{runInfo.repo_full_name} • {runInfo.branch} • {runInfo.commit_sha?.slice(0,7)}</div>}
        </div>
        <button id="security-analyze-btn" onClick={handleAnalyze} disabled={isAnalyzing}
          ...
          {isAnalyzing ? (<><svg className="animate-spin" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><circle cx="12" cy="12" r="10" strokeOpacity=".25"/><path d="M12 2a10 10 0 0 1 10 10"/></svg>Scanning...</>) : (<><span>🔍</span> Scan Now</>)}
        </button>
      </header>
